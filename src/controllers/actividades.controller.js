...
async function finalizarActividad(req, res) {
    ...
    try {
        const {id_usuario, id_actividad, inicio, fin} = req.body;
        const connection = await getConnection();
        const result = await connection.query('update usuarios_actividades set fecha_fin = "'+fin+'" where usuarios_id_usuario = "'+id_usuario+'" and actividades_id_actividad = '+id_actividad+' and fecha_fin is null and fecha_inicio = "'+inicio+'"');
